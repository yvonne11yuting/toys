...
const Notes = async () => {
  ...
  let publicData = data.filter(
    (note: any) =>
      note.readPermission !== "owner" && !!note?.lastChangeUser?.photo,
  );
  ...
  return (
    <div className="flex flex-col gap-2">
      {publicData.map((item: any) => {
        const createdAtStr = new Date(item.createdAt).toLocaleDateString(
          "zh-TW",
        );
        return (
          <div key={item.id} className="p-5">
            <div className="flex items-center text-sm gap-1">
              <Image
                src={item.lastChangeUser.photo}
                width={20}
                height={20}
                alt={`${item.lastChangeUser.name}'s photo`}
