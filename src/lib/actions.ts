...
export const getNote = async () => {
  try {
    const response = await fetch(
      `${serverUrl}/api/notes/650aca0984cea96311cee68c`,
      {
        method: "GET",
        headers: {
          "content-type": "application/json",
...
export const getPhotos = async () => {
  ...
  const apiVerKey =
    "AKfycbwqEWBMBuxVTTEWSzgekY1g08G1XD2fauTUlfkzAYsTcyQit2T1rM8L4FbpwEtrMi98oA";
  const serviceId = "yvonne-dev-test";
  const res = await axios.get(
    `${API_BASE_PATH}${apiVerKey}/exec?api=display-record&serviceId=${serviceId}`,
  );
